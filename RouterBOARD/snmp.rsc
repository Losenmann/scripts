...
:local vAuth; :local vEnc; :local vInterBr
	...
	set contact=$vContact location=$vLocat engine-id=89 trap-community=[/snmp community get 0 name] trap-version=3 enabled=yes;
	/ip firewall filter
	...
	print; :if ([find chain="input" protocol="udp" dst-port=161 in-interface=$vInterBr] = "") do={
		...
		else={chain="input" protocol="udp" dst-port=161 in-interface=$vInterBr action="accept" place-before=1 comment="SNMP Allow"}};
